...
private:
    ...
    Pos pos_;
public:
    Token(TokenKind kind, Pos pos):kind_(kind),value_(std::monostate{}),pos_(pos){}

    template<typename T>Token(TokenKind kind, T&& value, Pos pos)
        :kind_(kind),value_(std::forward<T>(value)),pos_(pos) {}

    TokenKind kind() const noexcept { return kind_;}
    const Pos& pos() const noexcept {return pos_;}
    ...
    template <typename T>bool is() const noexcept {return std::holds_alternative<T>(value_);}
    template <typename T>
...
inline std::string to_string(TokenKind kind){
    switch(kind){
        ...
        case TokenKind::RPAR: return ")";
        case TokenKind::LPAR: return "(";
        ...
        case TokenKind::COLON_ASSIGN: return ":=";
