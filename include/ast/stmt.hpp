...
struct IfStmt final : Stmt {
    ...
    IfStmt(std::unique_ptr<Expr> cond, std::unique_ptr<Block> then_b, std::unique_ptr<Block> else_b,
           Pos pos)
        : Stmt(NodeKind::IfStmt, pos), condition(std::move(cond)), then_branch(std::move(then_b)),
...
struct VarStmt final : Stmt {
    ...
    VarStmt(std::string name, TokenKind type_kind, std::unique_ptr<Expr> value, Pos pos)
        : Stmt(NodeKind::VarStmt, pos), name(std::move(name)), var_type_kind(type_kind),
